...
**阶段 D ~ N：概览 / 文章 / 分类 / 友链 / 友链申请 / 分享 / 关于页 / 音乐 / 回收站 / 发布 / 操作日志 / 备份与搜索 / 动效打磨 / 访问统计**
...
- ✅ **收尾与异常处理（阶段 N）** —— 这一轮是「先审计、再修」：写脚本把会静默失败 / 会卡死的地方量出来，抓到 4 个真问题全修掉
  · **会话过期（401）** → 自动回登录页并写明「登录已过期（原因）」；不再把人留在打不开的页面上
  · **接口 30 秒超时** → 提示「请求超时（30 秒没有响应）」；**断网 / 服务没启动** → 「连不上后台服务」（不再是英文的 Failed to fetch）
  · **接口 500** → 页面给人话错误，**不再卡在「正在读取…」**（文章 / 分类 / 回收站 / 操作日志都修了）
  · **操作日志按字节预算轮转**（>512KB 保留最新一批，原子替换），读日志只读文件尾部（不再全量进内存）
  · **空数据是合法状态**：空歌单 / 空友链 / 空分享都能正常打开（歌单空了会提示去 `music.json` 加一首）
  · **备份 zip 真能还原**：解压出来与仓库逐文件逐字节一致（含 MANIFEST 的 bytes / sha256）
