...
- ⚠️ **写范围受限**：文章只写 `src/content/blog/*.md`；分类只写 `src/data/categories.json`；删除进 `.admin-trash/`（已 gitignore）
- ⚠️ **「保存」≠「上线」**：保存只是写进仓库文件；真正发布（git push + Vercel Deploy Hook）是**阶段 I**，现在列表上会显示「N 个文件待发布」
- ⬜ 还没做：友链/分享/关于（G）、音乐（H）、发布与回滚（I）、申请审批（J）、统计（M）
